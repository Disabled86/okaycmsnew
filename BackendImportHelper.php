...
class BackendImportHelper
{
    ...
    public function importItem($item)
    {
        ...
        if(!empty($variantId) && !empty($productId)) {
            ...
            $importedItem->variant = $variantsEntity->findOne(['id' => $productId]);
    ...
    private function parseVariantData($itemFromCsv)
    {
		...
		if (isset($itemFromCsv['purchaseprice'])) {
            $purchaseprice = str_replace(',', '.', str_replace(' ', '', trim($itemFromCsv['purchaseprice'])));
            if (!empty($purchaseprice) || $purchaseprice === '0.00' || $purchaseprice === '0.0' || $purchaseprice === '0') {
        ...
        if (isset($itemFromCsv['barcode'])) {
            $variant['barcode'] = trim($itemFromCsv['barcode']);
        }
		
			
				if (isset($itemFromCsv['purchaseprice'])) {
            $variant['purchaseprice'] = trim($itemFromCsv['purchaseprice']);
        }
				
        if (isset($itemFromCsv['typeprefix'])) {
            $variant['typeprefix'] = trim($itemFromCsv['typeprefix']);
        }
		
		        if (isset($itemFromCsv['model'])) {
            $variant['model'] = trim($itemFromCsv['model']);
        }
		
		
		if (isset($itemFromCsv['newid'])) {
            $variant['newid'] = trim($itemFromCsv['newid']);
        }
		if (isset($itemFromCsv['id_aprel'])) {
            $variant['id_aprel'] = trim($itemFromCsv['id_aprel']);
        }

		if (isset($itemFromCsv['installationfunction'])) {
            $variant['installationfunction'] = trim($itemFromCsv['installationfunction']);
        }

		if (isset($itemFromCsv['installationkit'])) {
            $variant['installationkit'] = trim($itemFromCsv['installationkit']);
        }

		if (isset($itemFromCsv['toiletinstailationtype'])) {
            $variant['toiletinstailationtype'] = trim($itemFromCsv['toiletinstailationtype']);
        }

		if (isset($itemFromCsv['kit'])) {
            $variant['kit'] = trim($itemFromCsv['kit']);
        }

		if (isset($itemFromCsv['producttype'])) {
            $variant['producttype'] = trim($itemFromCsv['producttype']);
        }

		if (isset($itemFromCsv['category_av'])) {
            $variant['category_av'] = trim($itemFromCsv['category_av']);
        }

		if (isset($itemFromCsv['chandeliertype'])) {
            $variant['chandeliertype'] = trim($itemFromCsv['chandeliertype']);
        }

		if (isset($itemFromCsv['ligitingtype'])) {
            $variant['ligitingtype'] = trim($itemFromCsv['ligitingtype']);
        }

		if (isset($itemFromCsv['goodstype'])) {
            $variant['goodstype'] = trim($itemFromCsv['goodstype']);
        }

		if (isset($itemFromCsv['goodssubtype'])) {
            $variant['goodssubtype'] = trim($itemFromCsv['goodssubtype']);
        }
		...
		if (isset($itemFromCsv['chandeliermountingtype'])) {
            $variant['chandeliermountingtype'] = trim($itemFromCsv['chandeliermountingtype']);
        }
		...
		if (isset($itemFromCsv['id_dmitr'])) {
            $variant['id_dmitr'] = trim($itemFromCsv['id_dmitr']);
        }

		if (isset($itemFromCsv['id_dolg'])) {
            $variant['id_dolg'] = trim($itemFromCsv['id_dolg']);
        }		
		
		if (isset($itemFromCsv['id_domod'])) {
            $variant['id_domod'] = trim($itemFromCsv['id_domod']);
        }
		if (isset($itemFromCsv['id_drez'])) {
            $variant['id_drez'] = trim($itemFromCsv['id_drez']);
        }

		if (isset($itemFromCsv['id_dub'])) {
            $variant['id_dub'] = trim($itemFromCsv['id_dub']);
        }

		if (isset($itemFromCsv['id_egor'])) {
            $variant['id_egor'] = trim($itemFromCsv['id_egor']);
        }

		if (isset($itemFromCsv['id_zhuk'])) {
            $variant['id_zhuk'] = trim($itemFromCsv['id_zhuk']);
        }

		if (isset($itemFromCsv['id_zara'])) {
            $variant['id_zara'] = trim($itemFromCsv['id_zara']);
        }

		if (isset($itemFromCsv['id_zven'])) {
            $variant['id_zven'] = trim($itemFromCsv['id_zven']);
        }

		if (isset($itemFromCsv['id_ivant'])) {
            $variant['id_ivant'] = trim($itemFromCsv['id_ivant']);
        }		
		
		if (isset($itemFromCsv['id_istra'])) {
            $variant['id_istra'] = trim($itemFromCsv['id_istra']);
		}	
		if (isset($itemFromCsv['id_kashira'])) {
            $variant['id_kashira'] = trim($itemFromCsv['id_kashira']);
}
		if (isset($itemFromCsv['id_klin'])) {
            $variant['id_klin'] = trim($itemFromCsv['id_klin']);
}
		if (isset($itemFromCsv['id_kolom'])) {
            $variant['id_kolom'] = trim($itemFromCsv['id_kolom']);			
        }				
		if (isset($itemFromCsv['producer'])) {
            $variant['producer'] = trim($itemFromCsv['producer']);
        }
		if (isset($itemFromCsv['name_istra'])) {
            $variant['name_istra'] = trim($itemFromCsv['name_istra']);
        }
		
				if (isset($itemFromCsv['volume'])) {
            $variant['volume'] = trim($itemFromCsv['volume']);
        }
    ...
    private function prepareAddVariant($variant)
    {
        ...
        if (!isset($variant['price'])) {
            $variant['price'] = 0;
		        ...
		        if (!isset($variant['purchaseprice'])) {
            $variant['purchaseprice'] = 0;
        }
		
				        if (!isset($variant['producer'])) {
            $variant['producer'] = 0;
        }
